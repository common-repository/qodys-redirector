...
$type_slug = $this->GetClass('posttype_redirect-record')->m_type_slug;

$records = $wpdb->get_results( "SELECT * FROM $wpdb->posts WHERE post_type = '$type_slug' ORDER BY post_date DESC" );
...
if( $records )
{
	...
	foreach( $records as $key => $value )
	{
		...
		$ip = get_post_meta( $value->ID, 'ip_address', true );
		$redirect_url = get_post_meta( $value->ID, 'redirect_url', true );
		$host_id = get_post_meta( $value->ID, 'host_id', true );
		
		$new_record['data'] = $value;
		$new_record['date'] = $date;
		$new_record['ip'] = $ip;
		$new_record['redirect_url'] = $redirect_url;
		$new_record['host_id'] = $host_id;
		
		$records_by_latest[] = $new_record;
		
		$records_by_pages[ $host_id ]['all_time']++;
		$records_by_url[ $redirect_url ]['all_time']++;
		$records_by_ip[ $ip ]['all_time']++;
		
		// Today
		if( $date > $today && $date < $today+$day )
		{
			$records_by_pages[ $host_id ]['today']++;
			$records_by_url[ $redirect_url ]['today']++;
			$records_by_ip[ $ip ]['today']++;
		}
		
		// Yesterday
		if( $date > $yesterday && $date < $yesterday+$day )
		{
			$records_by_pages[ $host_id ]['yesterday']++;
			$records_by_url[ $redirect_url ]['yesterday']++;
			$records_by_ip[ $ip ]['yesterday']++;
		}
	}
}
?>
							...
							if( $records_by_latest )
							{
								...
								foreach( $records_by_latest as $key => $value )
								{
									...
									if( $value['host_id'] == -1 )
										...
										$title = get_the_title( $value['host_id'] );
								...
								<tr class="<?php echo $class; ?>">
									<td>
										<a href="<?php echo get_permalink($value['host_id']); ?>" target="_blank">
											<?php echo $title; ?>
										</a>
									</td>
									<td class="link_small">
									...
									<td style="text-align:center;">
										<?php echo $this->NumberTimeToStringTime( $timeNow - $value['date'] ); ?> ago
									</td>
								</tr>
								<?php
								}
